...
# TODO: change
def plot_graph(time_lag, X_LSTM, X_RNN, X_MLP, Y):
	fig, ax = plt.subplots()  # Create a figure and an axes.
	ax.set_xlabel('training examples')
	ax.set_ylabel('cross entropy')
	ax.set_title("time lag = " + str(time_lag))  # Add a title to the axes.

	ax.plot(Y, X_LSTM, label='LSTM')
	ax.plot(Y, X_RNN, label='RNN')
	ax.plot(Y, X_MLP, label='MLP')
...
def main():
	all_models_losses = []
	trained_on_vector = [(step + 1) * batch_size for step in range(iter)]
	X, Y = copy_data(T, K, n_train)
	ohX = torch.FloatTensor(batch_size, T + 2 * K, n_characters)
	models = [MLPModel, RNNModel, LSTMMODEL]
	...
	for class_model in models:
		...
		for step in range(iter):
			...
			curr_model_losses.append(loss.item())
		all_models_losses.append(curr_model_losses)

	plot_graph(T, all_models_losses[2], all_models_losses[1], all_models_losses[0], trained_on_vector)
