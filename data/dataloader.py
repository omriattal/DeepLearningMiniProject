...
class MyDataset(Dataset):
    def __init__(self, data: torch.tensor, begin: int, end: int, time_steps: int):
...
def long_tensor_as_data(tensor, printable_id):
    id_printable = {v: k for k, v in printable_id.items()}
    return "".join([id_printable[int(t)] for t in tensor])


def load_data(file_name, splits_percents, batch_size, time_steps, device) -> Tuple[list, dict]:
    ...
    for i in range(len(splits) - 1):
        current_dataset = MyDataset(data_as_tensor, splits[i], splits[i + 1], time_steps)
        if i == 0:  # train
